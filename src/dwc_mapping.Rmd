...
occurrences %>%
  ...
  dplyr::select(locatie, omschrijving)
```

Create column `location_improved` composed of the first capital letter of each word in `omschrijving` and the number in `locatie`:

```{r improve_locations}
occurrences <-
  occurrences %>%
  dplyr::mutate(
    location_improved = if_else(
      !grepl("\\D", locatie),
      paste0(
...
Example of the improved locations:

```{r show_results_improved}
occurrences %>% 
  filter(!grepl("\\D", locatie)) %>%
  select(locatie, omschrijving, location_improved) %>%
  head(10)
```
...
occurrences %>%
  filter(grepl("\\D", locatie)) %>% 
  dplyr::select(locatie, omschrijving, location_improved) %>%
  head(10)
```
...
```{r show_changes}
occurrences %>% select(x, y, x_improved, y_improved)
```
